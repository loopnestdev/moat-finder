...
export async function loadCheckpoints(
  tickerSymbol: string,
): Promise<{ runId: string; steps: Map<number, Record<string, unknown>> } | null> {
 ...
 */
export async function clearCheckpoints(tickerSymbol: string, runId: string): Promise<void> {
  const { error } = await adminClient
    .from('research_checkpoints')
    .delete()
    .eq('ticker_symbol', tickerSymbol)
  ...
  if (error) {
    console.error(`[checkpoint] Failed to clear checkpoints for ${tickerSymbol} run ${runId}:`, error.message);
  } else {
    console.log(`[checkpoint] Cleared checkpoints for ${tickerSymbol} run ${runId}`);
  }
}
