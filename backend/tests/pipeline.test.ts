...
const step2Json = JSON.stringify({
  ...
  constraint_analysis: {
    type: "technology",
    controls_constraint: true,
    durability: "durable",
    value_chain_position: "Sole provider of proprietary ML orchestration layer",
    rent_capture:
      "High switching costs ensure margin accrues to the company via multi-year contracts",
    investable: true,
    who_relieves:
      "Only a hyperscaler with equivalent R&D spend could displace; 3+ year qualification timeline",
    window: "24-36 months before hyperscaler equivalents reach feature parity",
  },
});
    ...
    nodes: [
      {
        id: "1",
        type: "revenue",
        data: { label: "SaaS Revenue", detail: "$200M ARR" },
        position: { x: 0, y: 0 },
      },
      {
        id: "2",
        type: "customer",
        data: { label: "Enterprise A" },
        position: { x: 600, y: 0 },
      },
    ],
...
describe("runPipeline", () => {
  ...
  it("emits started + complete SSE events for each step and returns a PipelineResult", async () => {
    ...
    const events: ReturnType<
      EmitFn extends (e: infer E) => unknown ? () => E : never
    >[] = [];
    const emit: EmitFn = (e) => {
      events.push(e);
    };
  ...
  it("throws when a step returns non-JSON text", async () => {
    mockCreate.mockResolvedValueOnce(
      makeEndTurnResponse("This is not JSON at all"),
    );
