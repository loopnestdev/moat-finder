...
function MetricRow({
  ...
  return (
    <div className="flex justify-between items-baseline gap-2 py-1 border-b border-navy-700/50 last:border-0">
      ...
      <span
        className={`font-mono text-sm font-medium ${valueClass} text-right min-w-0 truncate`}
      >
        {value}
      </span>
    </div>
...
export default function ValuationTable({
  ...
  if (rows.length === 0) {
    return (
      <p className="text-sm text-cream-subtle italic font-body">
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 gap-2">
      {sorted.map((row) => {
        ...
        const { text: growthText, colour: growthColour } = fmtGrowth(
          row.yoy_growth,
        );
        const grossPct =
        ...
        return (
          ...
          >
            ...
            <div className="mb-2.5">
              <p
                className={`font-mono text-base font-bold leading-tight ${isSubject ? "text-gold" : "text-cream"}`}
              >
                {row.ticker}
              </p>
