...
export default function QuarterlyResults({ results }: QuarterlyResultsProps) {
  return (
    <div className="rounded-xl bg-navy-950 border border-navy-700 overflow-hidden">
      {/* Header */}
      <div className="px-4 pt-4 pb-3">
        <p className="font-mono text-xs text-gold/60 uppercase tracking-[0.2em]">
          Quarterly Results
        </p>
        <p className="font-body text-[11px] text-cream-subtle mt-0.5">Last 4 quarters</p>
      ...
      {!results || results.length === 0 ? (
        <p className="px-4 pb-4 font-body text-sm text-cream-muted italic">
          Quarterly data not available for this report. Run an update to fetch latest figures.
        </p>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex items-baseline gap-1.5 mb-1">
                  ...
                  <span className="font-mono text-[10px] text-cream-subtle">→</span>
                  <span className="font-mono text-xs text-cream font-medium">
                    {fmtRevenue(r.revenue_act)}
                  </span>
                  {revBeat && (
                    <span className={`font-mono text-[10px] ml-auto ${revBeat.colour}`}>
                      {revBeat.text}
                    </span>
                  )}
                </div>
                ...
                {r.revenue_growth !== null && (
                  <div className="flex items-baseline gap-1.5 mb-1 pl-[3.75rem]">
                    <span className="font-mono text-[10px] text-cream-subtle">YoY</span>
                    <span className={`font-mono text-xs font-medium ${revGrowth.colour}`}>
                      {revGrowth.text}
                    </span>
                  </div>
                )}
                ...
                <div className="flex items-baseline gap-1.5">
                  ...
                  {epsBeat && (
                    <span className={`font-mono text-[10px] ml-auto ${epsBeat.colour}`}>
                      {epsBeat.text}
                    </span>
                  )}
                </div>
