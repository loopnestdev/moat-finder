...
- **Filter bar state lives in Home.tsx**: `minScore`, `minUpside`, `minYoy`, `sectorFilter`, `minSectorHeat`, `sortBy` — all client-side, no query key changes. `filtered` is a derived array from `reportList`. The "Clear Filters" button is only shown when `isFiltered` is true (any non-default filter active).
- **Sector dropdown options are derived, not hardcoded**: `sectorOptions` in `Home.tsx` is computed from the distinct `hot_sector_match` values across `reportList` — the LLM outputs free text here (not a fixed enum), so this stays in sync as new sector labels appear. Do NOT hardcode the `HOT_SECTORS` backend list (`Energy, Power, AI, Space, Nuclear, Semiconductor, Robotics, Solar`) as the dropdown source — real data has many more distinct values than that list.
