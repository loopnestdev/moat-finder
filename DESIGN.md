...
> **Implementation note**: This document describes Stripe's design system as a reference. For moat-finder's actual implementation:
>
> - **Font**: Söhne is a proprietary font not available via CDN. We use **Plus Jakarta Sans** (Google Fonts, weights 200–800) as the closest geometric sans-serif substitute. `font-feature-settings: "ss01"` and `font-optical-sizing: auto` are applied globally. Global weight is `300` with h1–h3 overriding to `700`.
> - **Dark mode**: Stripe's system is light-mode. moat-finder is entirely dark. We treat the whole app as Stripe's "brand dark section" (`#1c1e54` surfaces, `#0d1b38` page background).
> - **Monospace**: `SourceCodePro` replaced with **JetBrains Mono** for financial data and mono labels.
> - **Preserved tokens**: score colors (`#10b981`, `#f59e0b`, `#ef4444`) and gold (`#d4a853`) are moat-finder–specific and override Stripe defaults where they conflict.
