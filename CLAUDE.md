...
- **Fix pipeline crash on `null` Step 1 competitors/customers** (`backend/src/services/pipeline.ts`, `backend/tests/pipeline.test.ts`): new research on tickers like `VIST` (Vista Energy) failed immediately after Discovery with `Cannot read properties of null (reading 'join')`. The Discovery LLM returns `null` — not `[]` — for `competitors`/`customers` on companies with no identifiable "top customers" (commodity producers especially), and `formatStep1Context()`, which every downstream step calls first, ran `.join()` straight on the null. Added `normaliseStep1()` to coerce the raw Step 1 JSON (arrays defaulted to `[]`, `competitors` entries without a `name` dropped, null string fields → `""`) before it is returned from `runStep1()` and written to the checkpoint, and hardened `formatStep1Context()` with `?? []` guards + an `n/a` placeholder (matching the existing guarded pattern in `buildSynthesisContext`). Also guarded the Step 5 completion log's `hot_sector_match.join()` against null — a second source of the same error (hit on `AGX`). Regression tests added.

### v0.9.1

- **Dependency security patch** (`package.json` in root, `frontend/`, `backend/`): `/security-scan` audit found 15 frontend + 8 backend + 2 root npm vulnerabilities, including a high-severity RCE in `react-router-dom` (CWE-502, CVSS 8.1 — installed 7.14.0 was in the vulnerable range) and a critical DoS in the `tar` package pulled in by the `supabase` CLI. Fixes: `react-router-dom` `^7.14.0` → `^7.18.2`, `postcss` `^8.5.8` → `^8.5.26` (frontend, both direct prod deps), root `supabase` CLI `^2.85.0` → `^2.112.0` (newer CLI drops the vulnerable `tar` dependency entirely), `vitest` `^3.1.1`/`^3.2.6` → `^3.2.7` in both frontend and backend (critical arbitrary-file-execution CVE in the Vitest UI server), `wrangler` `^4.81.1` → `^4.120.0` and `@cloudflare/vite-plugin` `^1.31.2` → `^1.51.1` (frontend dev tooling — resolved the miniflare/sharp/undici/ws chain), `vite` `^8.0.4` → `^8.2.1` (frontend). Added an `overrides` entry pinning `body-parser` to `^1.20.6` in `backend/package.json` since Express 4.22.2 still ships the vulnerable `~1.20.5` and no patched 4.x release exists yet — avoided a risky Express 5.x major bump for a low-severity DoS. All three projects re-verified clean (`typecheck`, `test`, `build`) after the bumps. Also checked the active Aug 2026 "Shai-Hulud" npm worm (`keyv`/`cacheable`/`flat-cache`/`file-entry-cache`, reached via `eslint`) — this repo's installed versions are below the poisoned majors and every caret range in the chain is capped below them, so it isn't exposed. One low-severity `esbuild` issue remains in `backend/` (Windows-only dev-server file read via `tsx`) — no upstream fix published yet.

### v0.9.0

- **Headline numbers default to Bull, not Base** (`NapkinMath.tsx`, migration `004_default_bull_scenario.sql`): home page cards and the Report page's Napkin Math dropdown both now default to the Bull scenario. `target_price`/`upside_percent` generated columns redefined to mirror `report_json.scenarios`'s Bull entry (checking all 3 positions for the label, not assuming order) with a Base/`napkin_math` fallback for pre-v2 reports. Applied retroactively to all existing reports — no re-research needed. `napkin_math` and the Step 3 prompt are untouched; only the two summary columns and the frontend default changed. New `moat.safe_numeric()` helper function replaces the duplicated regex-guard pattern.
...
- **Exchange map + context injection** (`backend/src/services/pipeline.ts`): New `ExchangeInfo` interface and `EXCHANGE_MAP` covering 17 exchanges across Asia-Pacific, Europe, and Americas (AX=ASX, T=TSE, KS=KRX, KQ=KOSDAQ, HK=HKEX, SI=SGX, NZ=NZX, L=LSE, DE=XETRA, PA=EPA, AS=AMS, BR=EBR, SW=SIX, ST=STO, TO=TSX, V=TSXV, SA=B3). US tickers with no suffix default to NYSE/NASDAQ USD. `parseTickerExchange()` splits ticker on `.` and looks up the suffix. `buildExchangeContext()` generates a `STOCK CONTEXT:` block (ticker, exchange name, country, currency, currency symbol, search query suggestion, currency enforcement rule). This block is injected into all 7 step prompts as the first line of each system/user message.
- **Ambiguous ticker protection** (`pipeline.ts` Step 1): Discovery prompt now explicitly instructs the AI to search for the company trading on the exact exchange in STOCK CONTEXT. Prevents the wrong-country company from being resolved for cross-listed tickers (e.g. Australian `EOS.AX` vs US `EOS`).
- **Currency enforcement in valuation steps** (`pipeline.ts` Steps 3 & 7): Step 3 (Valuation) and Step 7 (Synthesis) include an explicit rule that `target_price` must be in the stock's local currency as specified in STOCK CONTEXT. Prevents USD prices being used for JPY/KRW/AUD stocks.
- **Exchange badge in report hero** (`frontend/src/pages/Report.tsx`): When the ticker contains a dot suffix, a small amber badge displays the exchange abbreviation (e.g. "ASX", "TSE", "KRX") next to the ticker symbol in the report hero. `EXCHANGE_BADGE_LABELS` map covers all 17 exchanges; unknown suffixes fall back to the raw suffix string.
